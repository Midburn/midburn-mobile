  ...
  _renderTexts() {
    return (
      <View right={isRTL()} key={this.props.camp.campId}>
        <Text text60 black style={{writingDirection: isRTL() ? 'rtl' : 'ltr', fontWeight: '600'}}>{this.title}</Text>
        <Text text80 black marginT-12 style={{writingDirection: isRTL() ? 'rtl' : 'ltr', fontWeight: '500'}}>{this.description}</Text>
      </View>
    );
  }
